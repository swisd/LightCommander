...
dir = str(directory).replace('v2', '')
print(dir)
temp_dir = dir + '/base/' + 'tmp.cubic'

with open(temp_dir, "w") as _f:
    _f.write(str(' '))
    _f.close()
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                self.advance()
                # tokens.append(Token(TT_DECLARE, pos_start=self.pos))
            elif self.current_char == '?':
                self.advance()
            elif self.current_char == '$':
                self.skip_sign()
                # tokens.append(Token(TT_SIGN, pos_start=self.pos))
        ...
        while self.current_char != None and (self.current_char != '"' or escape_character):
            if escape_character:
                ...
                if self.current_char == '\\':
                    ...
                    self.advance()
                    continue  # The above statements are useless without this one.
                else:
...
class ImportNode:
    def __init__(self, pos_start, pos_end, node_to_import):
        self.pos_start = pos_start
        self.pos_end = pos_end

        self.node_to_import = node_to_import
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_run(self, exec_ctx):
        should_exit = False
        fn: object = exec_ctx.symbol_table.get("fn")

        if not isinstance(fn, String):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "First argument must be path in string form",
                exec_ctx
            ))
        ...
        if should_exit: return RTResult().success_exit(Number.null)
        return RTResult().success(Number.null)

    execute_run.arg_names = ["fn"]

    def execute_map(self, exec_ctx):
        list_: object = exec_ctx.symbol_table.get("list")
        function: object = exec_ctx.symbol_table.get("func")
        output = []
        ...
        _: object
        for _ in list_:
            output.append(function(_))
        return RTResult.success(output, None)

    execute_map.arg_names = ["list", "function"]

    def execute_rtrn_bool(self, exec_ctx):
        val = exec_ctx.symbol_table.get("value")

        if not isinstance(val, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"Argument {val} must be 1 or 0",
                exec_ctx
            ))

        val = val.value

        self.outputstr = String(BOOLS[val])

        if not str(BOOLS[val]) in BOOLS:
            print(self.outputstr)
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"Argument val:<{val}> error",
                exec_ctx
            ))

        return RTResult().success(self.outputstr)

    execute_rtrn_bool.arg_names = ["value"]

    def execute_open_read(self, exec_ctx):
        file = exec_ctx.symbol_table.get("fn")
        output = open(file, "r")
        return RTResult().success(output.read())

    execute_open_read.arg_names = ["fn"]

    def execute_open_write(self, exec_ctx):
        file = exec_ctx.symbol_table.get("fn")
        value = exec_ctx.symbol_table.get("value")
        output = open(file, 'w')
        output.write(value)
    ...
    execute_open_write.arg_names = ["fn", "value"]

    def execute_exit(self, code):
        return RTResult().success_exit(Number.null)
        print(f"Process complete. Exit: {code}")
...
BuiltInFunction.rtrn_bool = BuiltInFunction("rtrn_bool")
...
class Interpreter:
    ...
    def no_visit_method(self, node, context):
        raise Exception(f'No visit_{type(node).__name__} method defined in basic. src:<{context}>')
...
global_symbol_table.set("OUT", Number.out)
...
global_symbol_table.set("rtrn_bool", BuiltInFunction.rtrn_bool)
...
def run(fn, text):
    ...
    with open(temp_dir, "a") as _f:
        _f.write(str(result))
        _f.write("\n\x02")
        _f.write(str(result.value))
        _f.write("\n")
        _f.write(str(result.error))
        _f.write("\x03")
        _f.close()
...
class auxilliary:
    ...
    def __index__(self, start_pos, end_pos, idx_st, idx_end):
        self.start_pos = start_pos
        self.end_pos = end_pos
        self.idx_st = idx_st
        self.idx_end = idx_end
        self.idx_cur = self.idx_st
        
        while self.idx_cur < self.idx_end:
            self.idx_cur += 1
